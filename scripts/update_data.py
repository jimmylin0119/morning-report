...
def build_data():
    ...
    def to_ticker(items):
        return [{"sym": x["name"], "price": f"{x['price']:,.2f}",
                 "chg": f"{'+' if x['pct']>=0 else ''}{x['pct']:.2f}%",
                 "dir": "up" if x["pct"] >= 0 else "down"} for x in items]
...
def main():
    ...
    print(f"  ✓ 寫入完成：{data['updated_at']}")
    print(f"  ✓ 市場狀態：{data['market_status']}")
