...
- 連結必須用 Telegram HTML 格式：<a href="完整URL">點此查看</a>
- 嚴禁輸出「附連結」「(連結)」「[連結]」這類佔位符文字
- 每則引用的新聞，後面都必須跟上對應的 <a href="..."> 標籤，缺一不可

【輸出格式】
先輸出 JSON（===JSON_START=== 到 ===JSON_END===），再輸出 5 則 Telegram 訊息（以 <<<MSG_SPLIT>>> 分隔）。
使用 Telegram HTML 格式：<b>粗體</b>、<i>斜體</i>、<a href="URL">連結文字</a>。不使用 Markdown。"""
...
- 每則必須用以下精確格式附上真實連結：
  📰 <a href="完整URL">點此查看</a>
- URL 一律從上方 RSS 清單裡挑，禁止編造
- 禁止輸出「附連結」三個字，必須用 <a> 標籤
- 範例（請完全照這格式）：
  - 特斯拉財報：摩根士丹利下修目標價，市場存疑
    📰 <a href="https://finance.yahoo.com/news/xxx.html">點此查看</a>
...
【最後再次強調】
- 第 1、2、3 則的所有新聞引用，必須用 <a href="完整URL">點此查看</a> 包起來
- 不可以輸出「📰 附連結」這種文字，必須是真正可點的 <a> 標籤
- 第 4、5 則結尾必須加上免責聲明 + 網站連結
- 所有數字只能用上方提供的真實數據
...
def sanitize_html(text):
    """把不允許的 HTML tag escape 掉，但保留允許的 tag（含 a 標籤的 href 屬性）。"""
    text = text.replace("&", "&amp;")
    placeholders = {}
    ...
    def stash(m):
        ...
        # 取得 tag name（去掉 / 和屬性）
        tag_name = inner.lstrip("/").split()[0].lower() if inner.lstrip("/").strip() else ""
        if tag_name in ALLOWED_TAGS:
            key = f"§§{len(placeholders)}§§"
            placeholders[key] = full_tag
            return key
...
def fix_link_placeholders(text, news_data):
    """補救：如果 Gemini 還是輸出『附連結』之類佔位符，自動替換成真連結。"""
    # 收集所有可用連結
    all_links = []
    for items in news_data.values():
        for it in items:
            all_links.append(it.get("link", ""))
    ...
    # 替換常見佔位符
    placeholders = ["📰 附連結", "📰附連結", "(連結)", "[連結]", "附連結"]
    idx = 0
    for ph in placeholders:
        while ph in text and idx < len(all_links):
            url = all_links[idx]
            replacement = f'📰 <a href="{url}">點此查看</a>'
...
def main():
    ...
    # 補救：把佔位符替換成真連結
    messages = [fix_link_placeholders(m, news_data) for m in messages]
    # 過濾不允許的 HTML tag
